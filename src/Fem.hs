...
-- place les valeurs de la matrice de rigidité locale dans une matrice qui fait la taille d'une globale, 
-- en fonction des indices des points de la liaison concernée.
...
-- Rigidity matrix
-- Somme les matrices de rigidité locales (issues de partiel & localRigidity) pour chacune des liaisons de la structure
...
-- produit de deux vecteurs [a1,a2,a3] [b1,b2,b3] = [a1*[b1,b2,b3], a2*[b1,b2,b3]...]	   
...
-- matrice 4*4 de rigidité locale calculée à partir du vecteur unitaire (source :)
...
--matrice de déplacement D  = K^-1 * F  . K matrice de rigidité global, F matrice des forces
deplacementMatrix :: Matrix -> Matrix -> Matrix
deplacementMatrix k f = mult (invert k) f

-- on a retiré les points fixes (premier et dernier points), il faut donc les remettre par la suite
-- to be improved...
...
--on change la structure de la liste de resultat pour qu'elle soit plus facilement lisible par l'interface
--
...
-- place les points au bon endroit/echelle dans le canvas  (à la java
--    --->x
--   |
-- y v
